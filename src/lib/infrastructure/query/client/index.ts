// src/lib/infrastructure/query/client/index.ts
...
export const prefetchQueries = {
  profile: async () => {
    await queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        ...
        const { userApi } = await import('@/lib/features/auth/api/auth.api');
  ...
  products: async (page = 1, limit = 10) => {
    await queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        const { productApi } = await import('@/lib/features/products/api/product.api');
